<template>
  <section class="mt-6 capitalize d_font">
    <div
      class="w-full block md:flex items-center justify-between border-b py-2"
    >
      <div
        class="text-lg w-full text-wrap __default_color dark:text-slate-800 flex justify-between flex-col"
      >
        <div class="w-auto !text-slate-200 flex">
          <div
            v-for="(cat, cat_idx) in item.item?.categories"
            v-bind:key="cat_idx"
          >
          ...
          <div
            v-if="item.item.categories?.length === 0"
        ...
        <a
          @click="handleClickDetail(item.item.id)"
          class="link link-hover font-bold __default_color !text-xl"
        >
          {{ item.item.name }}
          <!-- {{ item.item.name }} -->
        </a>
      ...
      <div
        class="w-[0.5px] h-full hidden md:block dark:text-slate-800 bg-gray-300 mx-6"
      ></div>
      <div class="flex w-5/5 md:w-[450px] justify-between mt-2 md:mt-0">
        <div
          class="flex w-full flex-col justify-center text-sm dark:text-slate-500 text-slate-500"
        >
          <div>
            <!-- {{ $t("last_updated_at") }}:
        ...
        <div
          class="h-full float-right pl-2 md:pl-4 flex flex-col justify-between"
        >
          <!-- modal download -->
          ...
          <div class="flex">
            <!--  -->
            <button
              class="btn btn-md mr-2 btn-success my-1 tooltip tooltip-left"
              :data-tip="$t('view_details')"
            ...
            <button
              class="btn btn-md btn-info bg-blue-600 border-blue-600 my-1 tooltip tooltip-left"
              :data-tip="$t('download')"
              @click="openModal(idx)"
            >
              <font-awesome-icon
                icon="download"
                class="text-md text-slate-200"
              />
            </button>
          </div>
          <!-- modal download  -->
          <dialog
            ref="modalDownload"
            :id="'modal' + idx"
            class="modal top-0 right-0"
          >
            <div class="modal-box !bg-slate-200 text-slate-800">
              <form method="dialog">
                <button
                  class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2 !text-red-600"
              ...
              <div class="!grid !grid-cols-4 gap-2 my-2">
                ...
                >
                  <img
                    src="../assets/images/icons8-adobe-acrobat-reader.gif"
                ...
                >
                  <img
                    src="../assets/images/icons8-document.gif"
                    alt="img-pdf-download"
                  />
                  <p class="text-sm">10kb</p>
                </div>
              ...
              <div class="!grid !grid-cols-1 gap-2 my-2 w-full">
                ...
                <div class="flex">
                  ...
                  <div class="w-auto !text-slate-200 flex  flex-wrap">
                    ...
                    >
                      <div
                        class="badge badge-info !text-slate-200 py-3 px-5 mb-2 mr-1"
                      >
                        {{ cat?.name ? cat?.name : "undefined" }}
                      </div>
                    </div>
